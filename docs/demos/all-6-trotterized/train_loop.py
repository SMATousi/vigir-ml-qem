...
def train(
    ...
    patience: int = 10,
    best_ckpt_path: str = "best_qem_graph_transformer.pt",
    scheduler_patience: int = 20,    # patience for LR scheduler
    scheduler_factor: float = 0.5,   # how much to reduce LR
    min_lr: float = 1e-6             # floor LR
):
    if device is None:
        device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    best = {"mae": float("inf"), "epoch": -1}
    history: List[Tuple[int, float, float, float, float]] = []  # (epoch, train_loss, val_loss, val_mae, lr)
    bad_epochs = 0
    ...
    for epoch in range(1, epochs + 1):
        ...
        history.append((epoch, train_loss, val_metrics["loss"], val_metrics["mae"], current_lr))
        print(f"Epoch {epoch:03d} | train {train_loss:.4f} | val {val_metrics['loss']:.4f} "
              f"| MAE {val_metrics['mae']:.4f} | lr {current_lr:.2e} | new")

        # ---- LR scheduler step (on val MAE) ----
